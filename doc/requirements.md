...
### 3.5 Discord メッセージの処理対象（人間 / Bot 投稿）

| 条件 | 挙動 |
|------|------|
| **既定**（`DISCORD_PROCESS_BOT_MESSAGES` 未設定または `false`） | **`author.bot === true` のメッセージはパースしない**（スパム回避・ループ回避のため） |
| **`DISCORD_PROCESS_BOT_MESSAGES=true`** | 監視チャンネル内の **Bot が投稿した売買報告も** Claude でパースし、DB に保存する |

- **コミュニティ運営での注意**: 講師・公式が **Bot アカウント（またはアプリ）で売買報告のみ**流すチャンネルでは、**本設定を `true` にしないとシグナル一覧に一切載らない**。手元検証で「自分の人間アカウントの投稿だけ通る」本番で「公式 Bot の投稿が通らない」という差は、ほぼこの挙動による。
- **自ワーカーの Bot** が同一チャンネルに投稿する場合のループは、`author.id === 自 Bot` のときは処理しない実装とする。

### 3.6 Turso 接続の安定性（ワーカー）

- ワーカーと Turso（例: `aws-ap-northeast-1`）の間で **一時的な `ETIMEDOUT` 等**が発生すると、その瞬間の **DB 書き込み（シグナル保存・価格監視）が失敗**しうる。
- 実装では **同一 SQL 実行を一定回数リトライ**する（指数バックオフ）。根本対策として **ワーカーのホストリージョンを Turso 近傍（例: Railway の Southeast Asia 等）に寄せる**ことを推奨。
...
| F-002 | Discord シグナル監視 | 指定チャンネル（最大10件、カンマ区切りID）の新着メッセージをリアルタイム取得。**既定は人間投稿のみ**；講師 Bot 投稿を含める場合は `DISCORD_PROCESS_BOT_MESSAGES=true`（§3.5） | 高 |
...
| `DISCORD_PROCESS_BOT_MESSAGES` | `true` のとき **Bot が投稿したメッセージも**パースする。未設定時は **人間の投稿のみ**（講師が Bot で流すコミュニティでは `true` が必要なことがある） | ワーカー（任意） |
...
8. **Discord**: 公式が **Bot 投稿**のコミュニティでは §3.5 の環境変数未設定により **シグナルが記録されない**ことがある
9. **DB ネットワーク**: ホストと Turso 間の **瞬断・タイムアウト**で保存に失敗しうる（§3.6）。リトライとリージョン選定で緩和
...
| **環境変数の確認** | `.env.local` がGitにコミットされていないこと、**Vercel** に本番用の値が設定されていること。常駐ワーカー（**Railway** 等）にも `TURSO_*`・`ENCRYPTION_KEY`（Vercel と同一）・`DISCORD_*`・`ANTHROPIC_API_KEY`、必要に応じ `PAPER_TRADING`・**`DISCORD_PROCESS_BOT_MESSAGES`**（講師が Bot 投稿のとき `true`）を設定 | シークレットの誤公開防止・ワーカー未設定によるクラッシュ防止・公式 Bot 投稿の取りこぼし防止 |
...
- **Bot 投稿の扱い**: 既定では **人間アカウントの投稿のみ**パース。講師・公式が **Bot で売買報告を流す**チャンネルを監視する場合は **`DISCORD_PROCESS_BOT_MESSAGES=true`**（§3.5、§6.2）
...
- `DISCORD_PROCESS_BOT_MESSAGES` — （任意）`true` で **他 Bot の投稿も**パース（§3.5）

**実装補足**: Turso への `execute` は **一時的ネットワークエラー時に自動リトライ**（§3.6）。
...
*作成日: 2025年3月14日*
*更新日: 2026年3月29日*
*バージョン: 7.3*（§3.5 Discord Bot 投稿、`DISCORD_PROCESS_BOT_MESSAGES`、§3.6 Turso 接続・リトライ、リスク・F-002・§13 整合）
