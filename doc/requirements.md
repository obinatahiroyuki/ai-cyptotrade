...
| 常駐ワーカー | Node.js (tsx) | Discord Bot + 価格監視（ローカル/VPS） |
...
| フェーズ | 稼働場所 | 目的 |
|----------|----------|------|
| **検証・開発** | ローカル (`npm run worker`) | 動作確認、テストネット |
| **本番** | VPS / AWS ECS 等 | 24/7安定稼働 |
...
| `DISCORD_CHANNEL_IDS` | 監視対象チャンネル ID（最大10件、カンマ区切り） | ワーカー |
...
### Phase 4: 運用・拡張（未着手）
- [ ] 本番口座での少額動作検証（Bitget スポットはテストネット非対応）
...
- `ENCRYPTION_KEY` — APIキー復号
- `DISCORD_BOT_TOKEN` — Discord Bot ログイン
- `DISCORD_CHANNEL_IDS` — 監視対象チャンネル（最大10件、カンマ区切り）
...
*作成日: 2025年3月14日*
*更新日: 2026年3月16日*
*バージョン: 7.1*（購入対象の判別（entry/achievement/other）、追加投資・損切りは実際の価格で判定、複数チャンネル監視、既保有時は購入しない）
